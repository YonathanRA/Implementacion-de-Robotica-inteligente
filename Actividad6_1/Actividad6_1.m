...
if op==1
% Hexagono
% MAs grande para que se pueda apreciar mejor
theta = deg2rad([0 60 120 180 240 300 360]);
vx = 15 * cos(theta);
vy = 15 * sin(theta);
...
% Flor
a = 50;      % Amplitud
k = 9;       % Número de pétalos (2k = 8 pétalos)
hxd = a * cos(k*t) .* cos(t);
hyd = a * cos(k*t) .* sin(t);


hxdp = -a * (k*sin(k*t).*cos(t) + cos(k*t).*sin(t));
hydp = -a * (k*sin(k*t).*sin(t) - cos(k*t).*cos(t));
...
axis([-60 60 -60 60 0 1]); % Ingresar limites minimos y maximos en los ejes x y z [minX maxX minY maxY minZ maxZ]
